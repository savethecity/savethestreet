...
function UniqueIDGenerator (init) {
    // Increments the id everytime it is called.
    var lastID=-1;
    if (init)
        lastID = init-1;
...
Sphere.prototype.setStatus = function(newstatus){
    /* utility function to change the status of a sphere. Possible states are hardcoded at the top
     of this file.
     Todo: figure out a good way to control also state transitions. I.e., no-one should go from
     dead to healthy or similar... */
    ...
    if (newstatus == STATUS_HEALTHY) {
        ...
        throw new Error("Unknow status "+newstatus + ".");
    }
    // Update the color in the animation:
    hiddenContext.fill();
...
Sphere.prototype.propagateVelVerlet = function(dt) {
    ...
    // Also doing some checks on the velocity to have a bound on them.
